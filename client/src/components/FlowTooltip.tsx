    ...
    <Show when={props.isOpen() && props.data()}>
      <Portal>
        <div
          ...
          class="rounded border border-[var(--plc-border-strong)] bg-[var(--plc-surface)] px-3 py-2 text-xs text-[var(--plc-on-surface)] shadow-[var(--plc-menu-shadow)]"
          style={{
            position: "fixed",
            left: `${pos().left}px`,
            top: `${pos().top}px`,
        ...
        >
          <div class="mb-1 font-semibold text-[var(--plc-accent)]">
            {props.data()?.text}
          </div>
          <Show when={props.data()?.snippet}>
            <div class="mt-2 rounded border border-[var(--plc-border)] bg-[var(--plc-code-bg)] p-2 font-mono text-[10px] text-[var(--plc-code-text)] whitespace-pre-wrap">
              {props.data()?.snippet}
            </div>
          </Show>
          ...
          <Show when={props.data()?.scopeSnippet}>
            <div class="mt-3 text-[10px] font-semibold text-violet-700">
              Captured from:
            </div>
            <div class="mt-1 rounded border border-[var(--plc-border)] bg-[var(--plc-code-bg)] p-2 font-mono text-[10px] text-[var(--plc-code-text)] whitespace-pre-wrap">
